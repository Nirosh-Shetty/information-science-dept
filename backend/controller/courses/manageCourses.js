...
export const addCourse = async (req, res) => {
    try {
        ...
        if (existingCourse) {
            ...
            return res.status(200).json({ message: "Class name already exists in the course.", course: existingCourse });
        }
...
export const getCourseByClass = async (req, res) => {
    try {
        ...
        if (!className) {
            return res.status(400).json({ message: "className is required to fetch course." });
        }
        const courses = await Course.find({ className });
        if (courses.length === 0) {
            return res.status(404).json({ message: "No course found for this class." });
        }
...
export const updateCourseById = async (req, res) => {
    try {
        ...
        if (!id || !className) {
            return res.status(400).json({ message: "ID and className are required." });
        }
        ...
        if (!course.className.includes(className)) {
            return res.status(404).json({ message: `ClassName "${className}" not found in the course.` });
        }

        // If multiple classNames are present
        if (course.className.length > 1) {
            // Remove the specified className from the className array
            const updatedClassNames = course.className.filter((cls) => cls !== className);
            ...
            return res.status(200).json({
                message: "Class removed from the original course and a new course created.",
                updatedCourse,
                newCourse,
            });
        }
...
export const deleteCourseByClassName = async (req, res) => {
    try {
        ...
        if (!name || !subCode || !className) {
            return res.status(400).json({
                message: "Name, subject code, and className are required to delete a course."
            });
        }
        ...
        if (!course) {
            return res.status(404).json({ message: "Course not found with the provided name and subject code." });
        }

        // Check if the `className` exists in the array
        if (!course.className.includes(className)) {
            return res.status(404).json({ message: "Class name not found in the course." });
        }
